...
import { LOCATION_WHEN_IN_USE_EXPLANATION } from '@/lib/location-permissions'
...
export function LocationPurposeModal({ visible, onAllow, onMaybeLater }: Props) {
  ...
  return (
    <Modal visible={visible} animationType="fade" presentationStyle="fullScreen" onRequestClose={onMaybeLater}>
      <SafeAreaView style={[styles.root, { backgroundColor: theme.background }]} edges={['top', 'bottom']}>
        <View style={[styles.card, { backgroundColor: cardBg, borderColor: border }]}>
          <View style={styles.pinWrap}>
            <MaterialIcons name="place" size={44} color="#FFFFFF" />
          </View>
          <Text style={[styles.title, { color: theme.text }]}>Location</Text>
          <Text style={[styles.body, { color: theme.icon }]}>{LOCATION_WHEN_IN_USE_EXPLANATION}</Text>
          <Pressable
            onPress={onAllow}
